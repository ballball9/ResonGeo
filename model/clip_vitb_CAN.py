...
@register_model
def deit_base_clip(pretrained=True):
    model = DeiTModel(
        num_patches=51,
        embed_dim=768,
        num_heads=4,
    ...
    if pretrained:
        ...
        for name_m, param_m in model.state_dict().items():
            ...
            if name_m_temp in checkpoint["model"]:
                ...
                if len(temp.shape) == 1:
                    temp = temp2[:temp.shape[0]]
                if len(temp.shape) == 2:
                    temp = temp2[:temp.shape[0], :temp.shape[1]]
                if len(temp.shape) == 3:
                    temp = temp2[:temp.shape[0], :temp.shape[1], :temp.shape[2]]
                if len(temp.shape) == 4:
                    temp = temp2[:temp.shape[0], :temp.shape[1], :temp.shape[2], :temp.shape[3]]

                model.state_dict()[name_m].copy_(nn.Parameter(temp))
    return model


def deit_large_clip(pretrained=True):
    model = DeiTModel(
        num_patches=258,
        embed_dim=1024,
        num_heads=4,
...
def deit_large_vit(pretrained=True):
    model = DeiTModel(
        num_patches=198,
        embed_dim=1024,
        num_heads=4,
...
def deit_huge_vit(pretrained=True):
    model = DeiTModel(
        num_patches=258,
        embed_dim=1280,
        num_heads=4,
